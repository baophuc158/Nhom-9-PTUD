...
<?php session_start(); ?>
     <style type="text/css">
		...
		#stud{
          ...
          <h1 class="page-header">Danh sách bài kiểm tra      <button type="text" class="btn btn-info" onclick="printContent('stud')" >
	...
	<tr>
		...
		<b><p>Danh sách bài kiểm tra môn</p></b>
  ...
  <table id="students" class="table table-bordered" >
    ...
    VALUES ('printed danh sách bài kiểm tra $tenmon','$user',NOW() )");
    $sql=  mysqli_query($conn, "SELECT * FROM hocsinh left join baikiemtratl on hocsinh.ma_baikttl = baikiemtratl.KT_ID WHERE monhoc = '$tenmon'");
     while($row = mysqli_fetch_assoc($sql)) {
        $sql2=  mysqli_query($conn, "SELECT * FROM bailamkttl left join hocsinh on bailamkttl.mahs=hocsinh.STUDENT_ID WHERE STUDENT_ID = '".$row['STUDENT_ID']."'");
            while($row2 = mysqli_fetch_assoc($sql2)) {

    ?>
    <form method="post" action="update_diemkt.php">
      <tr>
      <input type="hidden" name="id" value="<?php echo $row2['BAILAMKT_ID'] ?>">
        <td><?php echo $row['LASTNAME']?></td>
        <td><?php echo $row['CLASS'] ?></td>
        <td><?php echo $row2['bailam'] ?></td>
        <td><?php echo $row['cauhoi'] ?></td>
        <td><input  name="diemkt" type="text" style="border:0px" value="<?php echo $row2['diem'] ?>"></td>
